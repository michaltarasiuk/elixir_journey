defmodule ElixirJourney.Challenges.TwentyOnethMay2023 do
  ...
  defp unflatten_map(key, value) when is_binary(key) do
    splitted_key = split_key(key)

    put_in(%{}, splitted_key, value)
  end

  defp unflatten_map(term) when is_map(term) do
  ...
  # upperize
  defp upperize(term) when is_atom(term) do
    term
    |> Atom.to_string()
    |> String.upcase()
  end

  defp upperize(term) when is_map(term) do
    term
